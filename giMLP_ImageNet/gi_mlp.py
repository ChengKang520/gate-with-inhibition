...
class giMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.inhibition = nn.Linear(in_features, hidden_features)
        self.act1 = nn.LeakyReLU()
        self.gate = nn.Linear(hidden_features, out_features)
        self.act2 = nn.ReLU()
        self.drop = nn.Dropout(drop)
...
class CycleMLP(nn.Module):
    def __init__(self, dim, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.reweight = giMlp(dim, dim // 4, dim * 3)
...
class CycleBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = giMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer)
