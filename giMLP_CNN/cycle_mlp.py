...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., ratio_inhi=0.):
        super().__init__()
        ...
        self.threshold_ratio = ratio_inhi

    def forward(self, x, ):
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        threshold_inhi = torch.max(self.fc2(x)) * self.threshold_ratio
        x = self.fc2(x) - threshold_inhi
        x = self.drop(x)
        # print(self.threshold_ratio)
        return x

class giMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.act1 = nn.GELU()
        self.inhibition = nn.Linear(hidden_features, out_features)
        self.act2 = nn.GELU()
        self.drop = nn.Dropout(drop)
    ...
    def forward(self, x):
        ...
        x = self.act1(x)
        x = self.drop(x)
        threshold_inhi = torch.max(self.inhibition(x)) * threshold_ratio
        x = self.inhibition(x) - threshold_inhi
        x = self.act2(x)
        x = self.drop(x)
        return x
...
class CycleMLP(nn.Module):
    def __init__(self, dim, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., ratio_inhi=0.):
        super().__init__()
        ...
        self.reweight = Mlp(dim, dim // 4, dim * 3, ratio_inhi=ratio_inhi)
...
class CycleBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., ratio_inhi=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip_lam=1.0, mlp_fn=CycleMLP):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = mlp_fn(dim, qkv_bias=qkv_bias, qk_scale=None, attn_drop=attn_drop, ratio_inhi=ratio_inhi)
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, ratio_inhi=ratio_inhi)
...
def basic_blocks(dim, index, layers, mlp_ratio=3., qkv_bias=False, qk_scale=None, attn_drop=0.,
                 drop_path_rate=0., ratio_inhi=0., skip_lam=1.0, mlp_fn=CycleMLP, **kwargs):
    blocks = []

    for block_idx in range(layers[index]):
        block_dpr = drop_path_rate * (block_idx + sum(layers[:index])) / (sum(layers) - 1)
        blocks.append(CycleBlock(dim, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                      attn_drop=attn_drop, drop_path=block_dpr, skip_lam=skip_lam, mlp_fn=mlp_fn, ratio_inhi=ratio_inhi))
...
class CycleNet(nn.Module):
    """ CycleMLP Network """
    def __init__(self, layers, img_size=224, ther_ratio=0, patch_size=4, in_chans=3, num_classes=1000,
        embed_dims=None, transitions=None, segment_dim=None, mlp_ratios=None, skip_lam=1.0,
        qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., ratio_inhi=0., drop_path_rate=0.,
        ...
        for i in range(len(layers)):
            stage = basic_blocks(embed_dims[i], i, layers, mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias,
                                 ...
                                 norm_layer=norm_layer, skip_lam=skip_lam, mlp_fn=mlp_fn, ratio_inhi=ratio_inhi)
...
@register_model
def CycleMLP_B1(pretrained=False, ratio_inhi=0., **kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    model = CycleNet(layers, ratio_inhi=ratio_inhi, embed_dims=embed_dims, patch_size=7, transitions=transitions,
...
if has_mmseg and has_mmdet:
    ...
    class CycleMLP_B1_feat(CycleNet):
        def __init__(self, **kwargs):
            ...
            mlp_ratios = [4, 4, 4, 4]
            embed_dims = [64, 128, 320, 512]
            super(CycleMLP_B1_feat, self).__init__(layers, ratio_inhi=ratio_inhi, embed_dims=embed_dims, patch_size=7, transitions=transitions,
