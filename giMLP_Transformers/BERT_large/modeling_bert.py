...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if self.gi_key_side:
                if self.num_attention_heads != self.gi_attention_heads_i:
                    ...
                    key_states_head, key_states_tail = torch.split(encoder_hidden_states,
                    ...
                    key_layer_head = self.key(key_states_head)
                    key_layer_tail = torch.mul(selection_states_k, self.key(key_states_tail))
                    ...
                    if self.layer_i > self.num_hidden_layers - self.last_layers_num:
                        ...
                        threshold_inhi_k = torch.max(self.select_k(rehearsal_states_k)) * threshold_ratio
                        selection_states_k = nn.functional.gelu(
                            self.select_k(rehearsal_states_k) - threshold_inhi_k) + 1
                        key_layer = self.transpose_for_scores(
                            torch.mul(selection_states_k, self.key(encoder_hidden_states)),
                            self.num_attention_heads)
                    else:
                        key_layer = self.transpose_for_scores(self.key(encoder_hidden_states), self.num_attention_heads)
            else:
                key_layer = self.transpose_for_scores(self.key(encoder_hidden_states), self.num_attention_heads)
                ...
                if self.num_attention_heads != self.gi_attention_heads_i:
                    ...
                    if self.layer_i > self.num_hidden_layers - self.last_layers_num:
                        ...
                        selection_states_k = nn.functional.gelu(
                            self.select_k(rehearsal_states_k) - threshold_inhi_k) + 1
                        key_layer = self.transpose_for_scores(
                            torch.mul(selection_states_k, self.key(hidden_states)),
                            self.num_attention_heads)
                    else:
                        key_layer = self.transpose_for_scores(self.key(hidden_states), self.num_attention_heads)
            else:
                key_layer = self.transpose_for_scores(self.key(hidden_states), self.num_attention_heads)
        ...
        if self.gi_query_side:
            if self.num_attention_heads!= self.gi_attention_heads_i:
                self.segment_num = self.num_attention_heads - self.gi_attention_heads_i
                query_states_head, query_states_tail = torch.split(hidden_states, [self.segment_num, self.gi_attention_heads_i], dim=0)
                ...
                query_layer_head = self.query(query_states_head)
                query_layer_tail = torch.mul(selection_states_q, self.query(query_states_tail))
                ...
                if self.layer_i > self.num_hidden_layers - self.last_layers_num:
                  ...
                  threshold_inhi_q = torch.max(self.select_q(rehearsal_states_q)) * threshold_ratio
                  selection_states_q = nn.functional.gelu(self.select_q(rehearsal_states_q) - threshold_inhi_q) + 1
                  query_layer = self.transpose_for_scores(torch.mul(selection_states_q, self.query(hidden_states)), self.num_attention_heads)
                else:
                  query_layer = self.transpose_for_scores(self.query(hidden_states), self.num_attention_heads)
        else:
            query_layer = self.transpose_for_scores(self.query(query_states), self.num_attention_heads)
