...
class LoraConfig(PeftConfig):
    ...
    threshold_ratio_inhi: int = field(default=0.3, metadata={"help": "Lora Inhibition level"})
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        kwargs = {
            "r": lora_config.r,
            "threshold_ratio_inhi": threshold_ratio_inhi,
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer) and isinstance(target, torch.nn.Conv2d):
                target.update_layer_conv2d(
                    adapter_name,
                    lora_config.r,
                    lora_config.threshold_ratio_inhi,
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.threshold_ratio_inhi[self.active_adapter] == 0:
                result += (
                ...
                rehearsal_states_A = self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x))
                threshold_inhi_A = torch.max(rehearsal_states_A) * self.threshold_ratio_inhi[self.active_adapter]
                selection_states_A = nn.functional.gelu(rehearsal_states_A - threshold_inhi_A)
                selection_states_A = selection_states_A + 1
                result += (
                    self.lora_B[self.active_adapter](selection_states_A) * self.scaling[self.active_adapter]
                )
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
    ...
    def forward(self, x: torch.Tensor):
        if self.disable_adapters:
            ...
            if self.r[self.active_adapter] > 0:
                ...
                x = x.to(self.lora_A[self.active_adapter].weight.dtype)
                if self.threshold_ratio_inhi[self.active_adapter] == 0:
                    selection_states_A = self.lora_embedding_A[self.active_adapter].T
                else:
                    rehearsal_states_A = self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x))
                    threshold_inhi_A = torch.max(rehearsal_states_A) * self.threshold_ratio_inhi[self.active_adapter]
                    selection_states_A = nn.functional.gelu(rehearsal_states_A - threshold_inhi_A)
                ...
                if not torch.is_autocast_enabled():
                    ...
                    if x.dtype != torch.float32:
                        ...
                        if self.threshold_ratio_inhi[self.active_adapter] == 0:
                            ...
                            rehearsal_states_A = self.lora_A[self.active_adapter](
                                self.lora_dropout[self.active_adapter](x))
                            threshold_inhi_A = torch.max(rehearsal_states_A) * self.threshold_ratio_inhi[self.active_adapter]
                            selection_states_A = nn.functional.gelu(rehearsal_states_A - threshold_inhi_A)
                            selection_states_A = selection_states_A + 1
                            result += (
                                    self.lora_B[self.active_adapter](selection_states_A).to(expected_dtype)* self.scaling[
                                self.active_adapter]
                            )
                    ...
                    if self.threshold_ratio_inhi[self.active_adapter] == 0:
                        ...
                        selection_states_A = selection_states_A + 1
                        result += (
                                self.lora_B[self.active_adapter](selection_states_A) * self.scaling[
                            self.active_adapter]
                        )
