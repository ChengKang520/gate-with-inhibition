...
class SqueezeBertSelfAttention(nn.Module):
    def __init__(self, config, cin, q_groups=1, k_groups=1, v_groups=1):
        ...
        self.inhibition_k = nn.Conv1d(in_channels=cin, out_channels=cin, kernel_size=1, groups=k_groups)
        self.select_k = nn.Conv1d(in_channels=cin, out_channels=cin, kernel_size=1, groups=k_groups)
        # self.inhibition_q = nn.Conv1d(in_channels=cin, out_channels=cin, kernel_size=1, groups=q_groups)
        # self.select_q = nn.Conv1d(in_channels=cin, out_channels=cin, kernel_size=1, groups=q_groups)
    ...
    def forward(self, hidden_states, attention_mask, output_attentions):
        ...
        """
        # rehearsal_states_q = nn.functional.leaky_relu(self.inhibition_q(hidden_states))
        # selection_states_q = nn.functional.relu(self.select_q(rehearsal_states_q)) + 1
        # mixed_query_layer = torch.mul(selection_states_q, self.query(hidden_states))
        mixed_query_layer = self.query(hidden_states)

        rehearsal_states_k = nn.functional.leaky_relu(self.inhibition_k(hidden_states))
        selection_states_k = nn.functional.relu(self.select_k(rehearsal_states_k)) + 1
        mixed_key_layer = torch.mul(selection_states_k, self.key(hidden_states))
